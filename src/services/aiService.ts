...
function buildProjectAskDiagnosisInput(
  ...
  return JSON.stringify({
    ...
    repository_tree: context.repository_tree.slice(0, 220),
    readme: context.readme,
    package_json: context.package_json,
  });
}
...
function buildProjectAskDiagnosisInstructions(): string {
  return [
    ...
    "GROUNDED_IMPROVEMENT = prochaine amélioration de développement concrète et utile, justifiée par le code actuel, le but du projet, une décision active ou l'état observable du système, sans prétendre qu'un bug existe.",
    "INDETERMINATE = le contexte ne permet pas de trancher entre des prochaines étapes plausibles sans spéculation excessive.",
    "",
    "IMPORTANT POUR UNE QUESTION 'SUITE DU DÉVELOPPEMENT' : le fait que l'utilisateur demande une prochaine étape constitue déjà l'autorisation de recommander une amélioration. Il n'est PAS nécessaire qu'un bug ou un problème utilisateur ait été signalé.",
    "L'objectif n'est pas de trouver un défaut à tout prix. L'objectif est de choisir la prochaine évolution ayant le meilleur ancrage factuel dans le projet actuel.",
    "",
    ...
    "1. Liste mentalement les tâches actives pertinentes et les décisions actives.",
    "2. Cherche d'abord une tâche active réellement alignée avec la question et non déjà démontrée comme résolue.",
    "3. Si aucune tâche active ne convient, identifie les capacités actuellement présentes et le point d'évolution le plus directement justifié par elles.",
    "4. Formule une hypothèse précise sur le prochain besoin ou problème.",
    "5. Rassemble les preuves qui la soutiennent.",
    "6. Rassemble activement les éléments qui la réfutent ou la rendent moins certaine.",
    "7. Pour un BUG, suis la chaîne causale complète : appel → état → condition → await → retour → signal → destruction/effet, selon le cas.",
    "8. Pour une AMÉLIORATION, démontre plutôt la relation 'capacité actuelle → limite/gap observable → valeur de la prochaine évolution'. Une chaîne causale de bug n'est pas obligatoire.",
    "9. Pour une roadmap, une analyse statique du code et du contexte projet est suffisante ; l'absence d'exécution réelle ne doit pas empêcher une recommandation de développement.",
    "10. L'exécution, les logs runtime et les tests servent surtout à augmenter la certitude d'un défaut de comportement runtime ; ils ne sont pas requis pour justifier une GROUNDED_IMPROVEMENT.",
    "11. Distingue fait observé, inférence, hypothèse et préférence de conception.",
    "12. Si une étape critique manque pour affirmer un bug, choisis INDETERMINATE pour le bug ; cela n'interdit pas de recommander une amélioration clairement justifiée par d'autres preuves.",
    "",
    "RÈGLES IMPORTANTES :",
    ...
    "Une optimisation préventive n'est pas un VERIFIED_DEFECT, mais elle peut parfaitement être un GROUNDED_IMPROVEMENT si sa valeur et son ancrage dans le projet sont concrets.",
    "Une fonctionnalité absente n'est pas automatiquement un problème, mais son ajout peut être un GROUNDED_IMPROVEMENT lorsqu'il découle clairement du but du projet et de ses capacités actuelles.",
    "Une tâche historique terminée n'est pas une tâche actuelle.",
    "Une active_task pertinente reste prioritaire pour la planification tant que le code n'a pas démontré qu'elle est déjà résolue.",
    "Pour une question de roadmap, ne réponds pas 'aucun problème donc aucune modification'. Cherche la meilleure prochaine évolution objectivement justifiable.",
    "Si plusieurs améliorations sont plausibles, choisis celle qui a le meilleur ancrage dans les tâches actives, décisions actives, limites observables du code et cohérence avec le but du projet.",
    "INDETERMINATE doit être réservé au cas où aucune prochaine étape n'est suffisamment justifiable sans inventer des besoins ou des faits.",
    "Une confiance élevée exige une chaîne causale complète pour VERIFIED_DEFECT. Pour GROUNDED_IMPROVEMENT, elle exige surtout des preuves concrètes du système actuel et de la pertinence de l'évolution.",
    "N'invente jamais un problème utilisateur, une métrique de rétention, un besoin commercial ou une frustration qui ne figurent pas dans le contexte.",
    "",
...
function buildProjectAskDiagnosisReviewInstructions(): string {
  return [
    ...
    "Le mode de la question est PLANNING : l'utilisateur demande explicitement une prochaine étape de développement.",
    "Ton rôle n'est PAS d'exiger un bug, un crash ou une tâche déjà existante. Ton rôle est de vérifier que le diagnostic justifie correctement une prochaine évolution.",
    "Tu dois attaquer le raisonnement, pas le principe même de la demande de roadmap.",
    "",
    "CONTRAT DE REVIEW SELON LE STATUT :",
    "VERIFIED_DEFECT : vérifie la chaîne causale complète et rejette toute causalité spéculative.",
    "ACTIVE_TASK : vérifie que la tâche est réellement active et que le code fourni ne montre pas déjà qu'elle est résolue.",
    "GROUNDED_IMPROVEMENT : vérifie que la prochaine évolution est concrète, spécifique au projet et directement reliée à des capacités actuelles, au but du projet, à une décision active ou à une limite observable. Aucun bug préalable n'est requis.",
    "INDETERMINATE : acceptable uniquement lorsqu'aucune prochaine évolution raisonnable ne peut être sélectionnée sans inventer un besoin ou un fait.",
    "",
    "POUR UNE GROUNDED_IMPROVEMENT, pose ces questions :",
    "1. L'amélioration concerne-t-elle réellement ce projet précis ?",
    "2. Est-elle suffisamment concrète pour devenir une tâche de développement ?",
    "3. Peut-on expliquer son lien avec le gameplay ou les systèmes actuels ?",
    "4. Les preuves viennent-elles du code actuel, du but du projet ou d'une décision active ?",
    "5. Le diagnostic évite-t-il d'inventer une douleur utilisateur, un besoin business ou une métrique absente ?",
    "6. Existe-t-il une contre-preuve qui rend cette évolution incohérente ou inutile ?",
    "",
    "IMPORTANT :",
    "Le fait que le projet soit techniquement cohérent, compilable ou sans bug évident N'EST PAS une raison suffisante pour rejeter une GROUNDED_IMPROVEMENT.",
    "Une question 'quelle est la suite du développement ?' demande justement de choisir une évolution même en l'absence de défaut.",
    "Ne rejette jamais un diagnostic uniquement parce qu'il s'agit d'une recommandation de roadmap plutôt que d'une correction de bug.",
    "Un code source fourni n'a pas besoin d'être une application complète ou directement exécutable pour permettre une recommandation de roadmap.",
    "N'interprète pas un dump de fichiers comme un problème de formulation utilisateur : la question réelle est présente dans le champ question du contexte.",
    "",
    "REJETTE le diagnostic seulement si :",
    "- il invente un bug ou un comportement non démontré ;",
    "- il invente une fonctionnalité inexistante ;",
    "- il cite un fichier ou une décision absente ;",
    "- il repose sur un fichier tronqué pour démontrer une absence ;",
    "- il propose une amélioration générique sans ancrage projet ;",
    "- il ignore une contre-preuve importante qui rend la recommandation incohérente.",
    "",
    "APPROUVE si le statut est défendable après cette attaque.",
