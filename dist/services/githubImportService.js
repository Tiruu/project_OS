...
import { connectGithubRepository, getGithubRepositoryByRemote, } from "./githubRepositoryService.js";
...
async function githubFetch(url) {
    ...
    if (!response.ok) {
        throw new Error(`GitHub API ${response.status} sur ${url} : ${response.statusText}`);
    }
    return (await response.json());
}
async function tryGithubFetch(url) {
    try {
...
function extractDescription(repository, files, readmeContent) {
...
function detectType(technologies, files) {
    const fileNames = new Set(files.map((file) => file.name.toLowerCase()));
    if (fileNames.has("project.godot")) {
        return "game";
    }
    if (technologies.includes("Discord.js") ||
        technologies.includes("Discord")) {
        return "bot";
    }
    return "repository";
}
function detectTechnologies(repository, files, packageJson) {
...
export async function analyzeGithubRepository(owner, repository) {
    ...
    const files = (await tryGithubFetch(`${repositoryUrl}/contents?ref=${encodeURIComponent(remote.default_branch)}`)) ?? [];
    const readmeEntry = files.find((file) => file.type === "file" && /^readme(?:\.[^.]+)?$/i.test(file.name));
    const readmeFile = readmeEntry
        ? await tryGithubFetch(`${repositoryUrl}/contents/${encodeURIComponent(readmeEntry.path)}?ref=${encodeURIComponent(remote.default_branch)}`)
        : null;
    const packageJsonEntry = files.find((file) => file.type === "file" && file.name === "package.json");
    const packageJsonFile = packageJsonEntry
        ? await tryGithubFetch(`${repositoryUrl}/contents/${encodeURIComponent(packageJsonEntry.path)}?ref=${encodeURIComponent(remote.default_branch)}`)
        : null;
    ...
    const readmeContent = readmeFile
        ...
        : null;
    const technologies = detectTechnologies(remote, files, packageJson);
    return {
        owner,
        ...
        name: remote.name,
        description: extractDescription(remote, files, readmeContent),
        technologies,
        type: detectType(technologies, files),
    };
}
export async function importGithubProject(owner, repository) {
    const existingRepository = await getGithubRepositoryByRemote(owner, repository);
