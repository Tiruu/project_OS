...
const AI_REVIEW_CHUNK_LENGTH = DISCORD_MAX_CONTENT_LENGTH - AI_REVIEW_MARKER.length;
function splitDiscordMessage(content) {
    ...
    for (const line of content.split("\n")) {
        ...
        for (let offset = 0; offset < line.length; offset += AI_REVIEW_CHUNK_LENGTH) {
            chunks.push(AI_REVIEW_MARKER +
                line.slice(offset, offset + AI_REVIEW_CHUNK_LENGTH));
        }
        current = "";
    }
    if (current.length > 0) {
...
async function cleanupPreviousAiReviewMessages(interaction) {
    ...
    const previousMessages = messages.filter((message) => message.author.id === botId &&
    ...
    await Promise.all(previousMessages.map(async (message) => {
        try {
            ...
            console.warn("Impossible de supprimer une ancienne analyse IA :", error);
        }
    }));
}
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const [context, activities] = await Promise.all([
                getGithubRepositoryContext(repository.owner, repository.repository),
            ...
            const lines = [
                ...
                ...(review.observed_features.length > 0
                    ? review.observed_features.slice(0, 8).map((feature) => "• **" +
                        feature.name +
                        "** — " +
                        feature.description +
                ...
                ...(review.suggested_tasks.length > 0
                    ? review.suggested_tasks.map((task) => "• **" +
                        task.title +
                        "** [" +
                        task.task_kind +
                        "] — priorité " +
