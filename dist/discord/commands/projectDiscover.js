...
function buildComponents(repositories) {
    const available = repositories.filter((repository) => !repository.alreadyImported);
    const rows = [];
    for (let index = 0; index < Math.min(available.length, 25); index += 5) {
        ...
        for (const repository of available.slice(index, index + 5)) {
            row.addComponents(new ButtonBuilder()
                .setCustomId(`github-import:${repository.owner}/${repository.name}`)
                .setLabel(`Importer ${repository.name}`.slice(0, 80))
...
function buildMessage(owner, repositories) {
    const available = repositories.filter((repository) => !repository.alreadyImported);
    const imported = repositories.filter((repository) => repository.alreadyImported);
    const lines = [
    ...
    if (available.length > 25) {
        lines.push("", `... et ${available.length - 25} autre(s) dépôt(s).`);
    }
    return lines.join("\n");
}
export const projectDiscoverCommand = {
    ...
    async execute(interaction) {
        ...
        if (repositories.length === 0) {
            await interaction.editReply(`Aucun dépôt trouvé pour **${owner}**.`);
            return;
        }
    ...
    async handleImport(interaction) {
        const match = interaction.customId.match(/^github-import:([^/]+)\/(.+)$/);
        if (!match) {
        ...
        try {
            ...
            console.error(`Erreur import GitHub ${owner}/${repository} :`, error);
            await interaction.editReply({
