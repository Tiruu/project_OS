...
export const projectDoneCommand = {
    ...
    async execute(interaction) {
        ...
        await interaction.reply(`Tâche terminée dans le projet : **${task.title}**`);
    },
    async autocomplete(interaction) {
        try {
            const focused = interaction.options.getFocused(true);
            if (focused.name === "projet") {
                const { getProjects } = await import("../../services/projectService.js");
                const projects = await getProjects();
                const choices = projects
                    .filter((project) => project.name.toLowerCase().includes(focused.value.toLowerCase()))
                    .slice(0, 25)
                    .map((project) => ({
                    name: project.name,
                    value: project.id,
            ...
            if (focused.name === "tache") {
                ...
                const choices = tasks
                    ...
                    .filter((task) => task.title.toLowerCase().includes(focused.value.toLowerCase()))
                    .slice(0, 25)
                    .map((task) => ({
                    name: task.title,
                    value: task.id,
